...
If targeting an active conda environment then the `<install_directory>` can be set to `${CONDA_PREFIX}` in bash, or %CONDA_PREFIX% if using Anaconda Prompt on Windows.
