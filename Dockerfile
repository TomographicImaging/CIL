...
RUN sed -ri '/tigre|astra-toolbox/d' environment.yml \
  && for pkg in 'jupyter-server-proxy>4.1.0' $CIL_EXTRA_PACKAGES; do echo -e "\n  - $pkg" >> environment.yml; done \
...
RUN cmake -S ./src -B ./build -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCONDA_BUILD=ON -DCMAKE_INSTALL_PREFIX="${CONDA_DIR}" \
  && sed -i 's!#include <ipps.h>!#include <ipp/ipps.h>!p' src/src/Core/Binning.cpp src/src/Core/include/FBP_filtering.h \
