...
class FiniteDiff_cupy(LinearOperator):
    ...
    def __init__(self, gm_domain, gm_range=None, direction=0, bnd_cond = 'Neumann'):
        ...
        :param direction: optional axis in the icp.t :code:`DataContainer` along which to calculate the finite differences, default 0
        ...
        '''
        super(FiniteDiff_cupy, self).__init__() 
    ...
    def direct(self, x, out=None):
        ...
        if x_sz == 2:
            ...
            if self.direction == 1:
                ...
                cp.subtract( x_asarr[:,1:], x_asarr[:,0:-1], out = outa[:,0:-1] )
                
                if self.bnd_cond == 'Neumann':
                    pass
                elif self.bnd_cond == 'Periodic':
                    cp.subtract( x_asarr[:,0], x_asarr[:,-1], out = outa[:,-1] )
                else: 
    ...
    def adjoint(self, x, out=None):
        ...
        if x_sz == 2:        
            ...
            if self.direction == 1:
                ...
                cp.subtract( x_asarr[:,1:], x_asarr[:,0:-1], out = outa[:,1:] )
                
                if self.bnd_cond == 'Neumann':
                    cp.subtract( x_asarr[:,0], 0, out = outa[:,0] )
                    cp.subtract( -x_asarr[:,-2], 0, out = outa[:,-1] )
                    
                elif self.bnd_cond == 'Periodic':
                    cp.subtract( x_asarr[:,0], x_asarr[:,-1], out = outa[:,0] )                                        
