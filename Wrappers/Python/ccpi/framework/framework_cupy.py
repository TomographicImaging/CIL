    ...
    def allocate(self, value=0, dimension_labels=None, **kwargs):
        ...
        if isinstance(value, Number):
            ...
            if value == ImageGeometry.RANDOM:
                ...
                out.fill(cupy.random.random_sample(self.shape))
            elif value == ImageGeometry.RANDOM_INT:
                seed = kwargs.get('seed', None)
                if seed is not None:
                    cupy.random.seed(seed)
                max_value = kwargs.get('max_value', 100)
                out.fill(cupy.random.randint(max_value,size=self.shape))
            elif value is None:
                pass
            else:
...
class AcquisitionGeometry(object):
    ...
    def __init__(self, 
        ...
        angles is expected cupy array, dtype - float32
        ...
        if isinstance(angles, numpy.ndarray):
            ...
            raise ValueError('cupy array is expected')
...
class DataContainer(object):
    ...
    Data is currently held in a cupy arrays'''
    ...
    def subset(self, dimensions=None, **kw):
        ...
        if dimensions is None:
            ...
            if type(dimensions) == list:
                ...
                # "dimensions". 
                # create axes order for cupy.transpose
                axes = []
                ...
                cleaned = cupy.transpose(cleaned, axes).copy()
                if cleaned.ndim > 1:
    ...
    def fill(self, array, **dimension):
        '''fills the internal cupy array with the one provided
        
        :param array: cupy array to copy into the DataContainer
        :type array: DataContainer, cupy array or number
        :param dimension: dictionary, optional
        '''
        if dimension == {}:
            if issubclass(type(array), DataContainer) or\
               issubclass(type(array), cupy.ndarray):
                ...
                if issubclass(type(array), DataContainer):
                    cupy.copyto(self.array, array.array)
                else:
                    #self.array[:] = array
                    cupy.copyto(self.array, array)
            else:
    ...
    def __rpow__(self, other):
        if isinstance(other, (int, float)) :
            fother = cupy.ones(cupy.shape(self.array)) * other
    ...
    def pixel_wise_binary(self, pwop, x2, *args,  **kwargs):    
        ...
        if out is None:
            ...
            elif isinstance(x2, (cupy.int, cupy.int8, cupy.int16, cupy.int32, cupy.int64,\
                                 cupy.float, cupy.float16, cupy.float32, cupy.float64, \
                                 cupy.complex)):
                out = pwop(self.as_array() , x2 , *args, **kwargs )
            ...
            if self.check_dimensions(out) and self.check_dimensions(x2):
             ...
             isinstance(x2, (int,float,complex, cupy.int, cupy.int8, \
                             cupy.int16, cupy.int32, cupy.int64,\
                             cupy.float, cupy.float16, cupy.float32,\
                             cupy.float64, cupy.complex)):
        ...
        elif issubclass(type(out), cupy.ndarray):
    ...
    def add(self, other, *args, **kwargs):
        ...
        return self.pixel_wise_binary(cupy.add, other, *args, **kwargs)
    
    def subtract(self, other, *args, **kwargs):
        if hasattr(other, '__container_priority__') and \
           self.__class__.__container_priority__ < other.__class__.__container_priority__:
            return other.subtract(self, *args, **kwargs)
        return self.pixel_wise_binary(cupy.subtract, other, *args, **kwargs)

    def multiply(self, other, *args, **kwargs):
        if hasattr(other, '__container_priority__') and \
           self.__class__.__container_priority__ < other.__class__.__container_priority__:
            return other.multiply(self, *args, **kwargs)
        return self.pixel_wise_binary(cupy.multiply, other, *args, **kwargs)
    
    def divide(self, other, *args, **kwargs):
        if hasattr(other, '__container_priority__') and \
           self.__class__.__container_priority__ < other.__class__.__container_priority__:
            return other.divide(self, *args, **kwargs)
        return self.pixel_wise_binary(cupy.divide, other, *args, **kwargs)
    
    def power(self, other, *args, **kwargs):
        return self.pixel_wise_binary(cupy.power, other, *args, **kwargs)    
          
    def maximum(self, x2, *args, **kwargs):
        return self.pixel_wise_binary(cupy.maximum, x2, *args, **kwargs)
    
    def minimum(self,x2, out=None, *args, **kwargs):
        return self.pixel_wise_binary(cupy.minimum, x2=x2, out=out, *args, **kwargs)

    def axpby(self, a, b, y, out, dtype=cupy.float32, num_threads=NUM_THREADS):
        ...
        :type dtype: cupy type, optional, default cupy.float32
        ...
        if dtype == cupy.float32:
            ...
            raise TypeError('Unsupported type {}. Expecting cupy.float32 or cupy.float64'.format(dtype))
    ...
    def abs(self, *args,  **kwargs):
        return self.pixel_wise_unary(cupy.abs, *args,  **kwargs)
    
#    def max(self, *args,  **kwargs):
#        return self.pixel_wise_unary(cupy.max, *args,  **kwargs) 
#    
#    def min(self, *args,  **kwargs):
#        return self.pixel_wise_unary(cupy.min, *args,  **kwargs)     
    
    def sign(self, *args,  **kwargs):
        return self.pixel_wise_unary(cupy.sign, *args,  **kwargs)
    
    def sqrt(self, *args,  **kwargs):
        return self.pixel_wise_unary(cupy.sqrt, *args,  **kwargs)

    def conjugate(self, *args,  **kwargs):
        return self.pixel_wise_unary(cupy.conj, *args,  **kwargs)

    def exp(self, *args, **kwargs):
        '''Applies exp pixel-wise to the DataContainer'''
        return self.pixel_wise_unary(cupy.exp, *args, **kwargs)
    
    def log(self, *args, **kwargs):
        '''Applies log pixel-wise to the DataContainer'''
        return self.pixel_wise_unary(cupy.log, *args, **kwargs)
    ...
    def squared_norm(self):
        ...
        #y = cupy.reshape(self.as_array(), (size, ))
        return self.dot(self.conjugate())
        #return self.dot(self)
    def norm(self):
        '''return the euclidean norm of the DataContainer viewed as a vector'''
        return cupy.sqrt(self.squared_norm())
    
    
    def dot(self, other, *args, **kwargs):
        '''return the inner product of 2 DataContainers viewed as vectors'''
        method = kwargs.get('method', 'cupy')
        if method not in ['cupy','reduce']:
            raise ValueError('dot: specified method not valid. Expecting cupy or reduce got {} '.format(
                    method))

        if self.shape == other.shape:
            # return (self*other).sum()
            if method == 'cupy':
                return cupy.dot(self.as_array().ravel(), other.as_array().ravel())
            elif method == 'reduce':
    ...
    def min(self, *args, **kwargs):
        ...
        return cupy.min(self.as_array(), *args, **kwargs)
    
    def max(self, *args, **kwargs):
        '''Returns the max pixel value in the DataContainer'''
        return cupy.max(self.as_array(), *args, **kwargs)
        ...
        if array is None:
            ...
            if issubclass(type(array) , DataContainer):
                ...
                super(ImageData, self).__init__(array.as_array(), deep_copy,
                                 array.dimension_labels, **kwargs)
            elif issubclass(type(array) , cupy.ndarray):
    ...
    def subset(self, dimensions=None, **kw):
        ...
        if out.number_of_dimensions > 1:
            ...
            if sliceme is not None:
                angles = cupy.asarray([ self.geometry.angles[sliceme] ] , cupy.float32)
            else:
...
class CastDataContainer(DataProcessor):
    ...
    def process(self, out=None):
        ...
        dtype = self.dtype
        if out is None:
            y = cupy.asarray(dsi.as_array(), dtype=dtype)
            
            return type(dsi)(cupy.asarray(dsi.as_array(), dtype=dtype),
                                dimension_labels=dsi.dimension_labels )
        else:
            out.fill(cupy.asarray(dsi.as_array(), dtype=dtype))
...
class PixelByPixelDataProcessor(DataProcessor):
    ...
    def process(self, out=None):
        ...
        eval_func = cupy.frompyfunc(pyfunc,1,1)
...
class VectorData(DataContainer):
    ...
    def __init__(self, array=None, **kwargs):
        self.geometry = kwargs.get('geometry', None)
        self.dtype = kwargs.get('dtype', cupy.float32)
        ...
        if self.geometry is None:
            ...
            if array is None:
                out = cupy.zeros((self.length,), dtype=self.dtype)
            else:
