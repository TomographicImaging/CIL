...
class TestFunction(unittest.TestCase):
    ...
    def test_Function_strongly_convexity(self):
        
        # F is not strongly convex, cc of F is not strongle convex
        f = Function()
        self.assertIsNone(f.gamma)
        self.assertIsNone(f.gamma_conj)

        # change value
        f.gamma = 1.0
        f.gamma_conj = 2.0
        self.assertIsNotNone(f.gamma)
        self.assertIsNotNone(f.gamma_conj)
    ...
    def test_SumFunction_strongly_convexity(self):
        
        # Both functions not stronlgy convex
        f = Function()
        g = Function()
        h = f + g
        self.assertIsNone(h.gamma)
        self.assertIsNone(h.gamma_conj)

        # First is convex, second is strongly convex: Function itself
        f = Function()
        g = Function()
        g.gamma = 1.0
        h = f + g
        self.assertEquals(h.gamma, g.gamma)

        # First is strongly convex, second is convex: Function itself
        f = Function()
        f.gamma = 2.0
        g = Function()
        h = f + g
        self.assertEquals(h.gamma, f.gamma)  

        # Both strongly convex: Function itself
        f = Function()
        f.gamma = 2.0
        g = Function()
        g.gamma = 4.0
        h = f + g
        self.assertEquals(h.gamma, f.gamma + g.gamma)   
        
        # First is convex, second is strongly convex: Convex conjugate of the Function 
        f = Function()
        g = Function()
        g.gamma_conj = 1.0
        h = f + g
        self.assertEquals(h.gamma_conj, g.gamma_conj)

        # First is strongly convex, second is convex: Convex conjugate of the Function 
        f = Function()
        f.gamma_conj = 2.0
        g = Function()
        h = f + g
        self.assertEquals(h.gamma_conj, f.gamma_conj)  

        # Both strongly convex: Convex conjugate of the Function 
        f = Function()
        f.gamma_conj = 2.0
        g = Function()
        g.gamma_conj = 4.0
        h = f + g
        self.assertEquals(h.gamma_conj, f.gamma_conj + g.gamma_conj) 

    def test_ScaledFunction_strongly_convexity(self):

        # Scaled Function, gamma and gamma_conj default values are None
        scalar = 3
        f = Function()
        g = scalar * f
        self.assertIsNone(g.gamma)  
        self.assertIsNone(g.gamma_conj) 

        # Check strongly convexity
        f.gamma = 3.0
        f.gamma_conj = 2.0
        g = scalar * f
    ...
    def test_SumFunction_scalar_strongly_convexity(self):   
        
        # gamma and gamma_conj default values are None
        f = Function()
        g = f + 3
        self.assertIsNone(g.gamma)  
        self.assertIsNone(g.gamma_conj)   

        # gamma and gamma_conj 
        f.gamma = 3.0
        f.gamma_conj = 2.0
        g = f + 3
