...
class PD3O(Algorithm):
    ...
    def __init__(self, f, g, h, operator, delta=None, gamma=None, initial=None, **kwargs):
        ...
        self.set_up(f=f, g=g, h=h,  operator=operator, delta=delta,
                    gamma=gamma, initial=initial, **kwargs)
    ...
    def update(self):
        ...
        # x_old - gamma * grad_f(x_old)
        self.x_old.sapyb(1., self.grad_f, -self.gamma, out=self.grad_f)
        ...
        # 2*x - x_old + gamma*(grad_f_x_old) - gamma*(grad_f_x)
        self.x.sapyb(2, self.grad_f, -1.0,  out=self.x_old)
