...
class BlockDataContainer(object):
    ...
    1) algebra between `BlockDataContainer`s will be element-wise, only if 
       ...
       will fail 
    2) algebra between `BlockDataContainer`s and `list` or `cupy array` will 
...
#        shape = (len(args),1)
    ...
    def is_compatible(self, other):
        ...
        if isinstance(other, Number):
            return True   
        elif isinstance(other, (list, cupy.ndarray)) :
            for ot in other:
                if not isinstance(ot, (Number,\
                                 cupy.int, cupy.int8, cupy.int16, cupy.int32, cupy.int64,\
                                 cupy.float, cupy.float16, cupy.float32, cupy.float64, \
                                 cupy.complex)):
                    raise ValueError('List/ cupy array can only contain numbers {}'\
                                     .format(type(ot)))
    ...
    def binary_operations(self, operation, other, *args, **kwargs):
        ...
        elif isinstance(other, (list, cupy.ndarray, BlockDataContainer)):
            # try to do algebra with one DataContainer. Will raise error if not compatible
            kw = kwargs.copy()
            res = []
    ...
    def power(self, other, *args, **kwargs):
        ...
        elif isinstance(other, list) or isinstance(other, cupy.ndarray):
    ...
    def sum(self, *args, **kwargs):
        return cupy.sum([ el.sum(*args, **kwargs) for el in self.containers])
    
    def squared_norm(self):
        y = cupy.asarray([el.squared_norm() for el in self.containers])
        return y.sum() 
    ...
    def __radd__(self, other):
        ...
        to make sure that this method is called rather than the __mul__ of a cupy array
        the class constant __array_priority__ must be set > 0
        https://docs.scipy.org/doc/cupy-1.15.1/reference/arrays.classes.html#cupy.class.__array_priority__
        '''
        return self + other
    # __radd__
...
#    w1 = sum(U).as_array()    
#    cupy.testing.assert_array_equal(w, w1)
#    
#    print ("test sum BDC " )
#    z = cupy.sqrt(U[0].as_array()**2 + U[1].as_array()**2)
#    z1 = sum(U**2).sqrt().as_array()    
#    cupy.testing.assert_array_equal(z, z1)   
#    
#    z2 = U.pnorm(2)
#    
#    zzz = U.dot(V)
