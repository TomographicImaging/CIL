...
int fdiff_direct_neumann_L21sum(const float *inimagefull, float *outimageXfull, float *outimageYfull, float *outimageZfull, float *outimageCfull, long nx, long ny, long nz, long nc, float * L21_sum)
{
	...
	double * L21_arr = 	(double *)malloc(th_num * sizeof(double));
	memset(L21_arr, 0, th_num * sizeof(double));
	*L21_sum = 0.0;
...
#pragma omp for
				for (ind = 0; ind < volume; ind++)
				{
					L21_arr[omp_get_thread_num()] += outimageX[ind] * outimageX[ind] + outimageY[ind] * outimageY[ind] + outimageZ[ind] * outimageZ[ind];
				}
			}
			else
			{
				...
				for (ind = 0; ind < volume; ind++)
				{
					L21_arr[omp_get_thread_num()] += outimageX[ind] * outimageX[ind] + outimageY[ind] * outimageY[ind];
				}
			}
	...
	long double L21_sum_temp = 0;
	for(int t = 0; t < th_num; t++)
	{
		L21_sum_temp += L21_arr[t];
	}
	free(L21_arr);
...
#pragma omp for
				...
				{
					outimage[ind] = tempX[ind] + tempY[ind] + tempZ[ind];
					L21_arr[omp_get_thread_num()] += outimage[ind] * outimage[ind];
				}			
			}
			else
			{
...
int fdiff_adjoint_neumann_PA(float *bufferA, float *bufferB, float *bufferC, float *outimagefull, const float *inimageXfull, const float *inimageYfull, const float *inimageZfull, const float *inimageCfull, long nx, long ny, long nz, long nc)
{
	//runs over full data in x, y, z. then corrects elements for bounday conditions and sums
	size_t volume = nx * ny * nz;
...
DLL_EXPORT int fdiff3D_neu_f_L21sum(float *imagefull, float *gradZfull, float *gradYfull, float *gradXfull, long nz, long ny, long nx, int boundary, int direction, int nThreads, float * L21_sum)
{
	...
	fdiff_direct_neumann_L21sum(imagefull, gradXfull, gradYfull, gradZfull, NULL, nx, ny, nz, 1, L21_sum);
...
DLL_EXPORT int fdiff3D_neu_b_L21sum(float *imagefull, float *gradZfull, float *gradYfull, float *gradXfull, long nz, long ny, long nx, int boundary, int direction, int nThreads, float * L21_sum)
{
	...
	fdiff_adjoint_neumann_L21sum(imagefull, gradXfull, gradYfull, gradZfull, NULL, nx, ny, nz, 1, L21_sum);
