...
class SystemConfiguration(object):
    ...
    def geometry(self,val):
        if val != AcquisitionGeometry.CONE and val != AcquisitionGeometry.PARALLEL and val != AcquisitionGeometry.CONE_SOUV:
            raise ValueError('geom_type = {} not recognised please specify \'cone\', \'cone_souv\' or \'parallel\''.format(val))
        else:
            self._geometry = val
...
class AcquisitionGeometry(object):
    ...
    def set_angles(self, angles, initial_angle=0, angle_unit='degree'):
        ...
        if self.config.system.geometry == "cone_souv":
            raise ValueError("cone_souv systems do not use angles as they use per-projection geometries.");
